...
#include "dejian_spatialparams_poroelastic.hh"
...
//struct Grid<TypeTag, TTag::PoroElasticSub> { using type = Dune::YaspGrid<2, Dune::TensorProductCoordinates<double, 2>>; };
struct Grid<TypeTag, TTag::PoroElasticSub> { using type = Dune::YaspGrid<2, Dune::TensorProductCoordinates<double, 2>>;};
...
//        	values.setAllDirichlet();
        ...
        else if (onRightBoundary_ (globalPos) || onLeftBoundary_ (globalPos))
//        	values.setNeumann(Indices::momentum(0));
            values.setDirichlet(Indices::u(0));
        else
//        	values.setNeumann(Indices::momentum(1));
